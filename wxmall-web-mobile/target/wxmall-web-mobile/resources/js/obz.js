...
var obz = obz || {
    ...
    showMessage : function(message, okCallback, cancelCallback){
    	...
    	/*if ($("#__systemMessageDialog").length == 0) {
    		$("body").append('<div id="__systemMessageDialog"></div>');
    		$("#__systemMessageDialog").dialog({
    			autoOpen : false,
    			title : '提示',
    			modal : true,
    			width : 500,
    			...
    			close : function(){
    				$("#__systemMessageDialog").remove();    				
    			}
    		});
    	}
    	$("#__systemMessageDialog").html("" + message + "");
    	$("#__systemMessageDialog").dialog('open');
    	return false;*/
