...
public class IndexController extends BaseController {
	...
	final int loginFailureLockCount = 100;

	public void index() {
		/*
		 * Long qiandaoCount = Db.queryLong("select count(*) from " +
		 * Qiandao.table + " where active=1 and seller_id=? ", getSellerId());
		 * setAttr("qiandaoCount", qiandaoCount);
		 * 
		 * Long LotteryCount = Db.queryLong("select count(*) from " +
		 * Lottery.table + " where active=1 and seller_id=? ", getSellerId());
		 * setAttr("lotteryCount", LotteryCount);
		 * 
		 * /*Long prizeCount = Db.queryLong("select count(*) from " +
		 * Prize.table + " p " + "left join " +PrizeType.table+
		 * " t on p.prize_type_id=t.id where p.active=1 and p.seller_id=? and t.active=1 "
		 * , getSellerId()); setAttr("prizeCount", prizeCount);
		 */

		Long buyerCount = Db.queryLong("select count(*) from " + BuyerUser.table + " where active=1 and seller_id=? ",
				getSellerId());
		...
		Long productCount = Db.queryLong(
				"select count(*) from " + Product.table + " where active=1 and is_marketable=1 and seller_id=? ",
				getSellerId());
	...
	public void auth() {
		...
		try {
			...
			token.clear();
			int loginFailureCount = admin.getInt("admi_login_failure_count") == null ? 0
					: admin.getInt("admi_login_failure_count") + 1;
			...
			if (loginFailureLockCount - loginFailureCount <= 6) {
				rendFailedJson("若连续" + loginFailureLockCount + "次密码输入错误,您的账号将被锁定,还有"
						+ (loginFailureLockCount - admin.getInt("admi_login_failure_count")) + " 次机会。");
				return;
			} else {
	...
	public void doRegister() {
		...
		// check 手机验证码
		UserCode userCode = UserCode.dao
				.findFirst("select * from " + UserCode.table + " where vcode_phone=? and vcode_code=? ", phone, code);
		if (userCode == null) {
